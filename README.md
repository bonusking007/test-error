...
local Window = library:CreateWindow("Wiwat Private | CW Mobile", Vector2.new(400, 550), Enum.KeyCode.RightShift) 
...
local AASec = MiscTab:CreateSector("AUTOFARM", "right")
...
-- Function to force the character into Freefall state
local function forceFreefall()
    while true do
        ...
        if character then
            ...
            if humanoid then
                -- Set the character's state to Freefall
...
-- Call the function to force Freefall state
forceFreefall()
end)
